...
自作CPUが計算したレイトレの結果が、パソコンへ送信されます。青いLEDが光ったら、レイトレ完了です。

（動画スタート）

（青いLEDが光ったら）

青いLEDが光ったらレイトレ完了なので、出力された画像を確認します。
...
CPU実験の体験記が毎年WEBに投稿されるので、いつも楽しみにしています。
それらを読んでるうちに「自分でもやってみたい！」と思い、レイトレの移植を始めました。
-->
...
説明の都合上、分けて説明するのですが、実際の手順はこんなに厳密に分かれてはおらず、各手順を行ったり来たりしながら作業をすすめました。
-->
...
<!--
みなさん、お手持ちのFPGAボードで動くかどうか気になってるかと思いますが、こちらの条件を満たせば、お手持ちのFPGAボードでレイトレが動かせると思います。

まず、LUTが1万2000程度必要です。Tang Nano 9Kだと足りなくて、Tang Nano 20Kなら収まりそうです。

これは、私が作成したCPUのLUT数から求めた数字なので、うまいことLUTを圧縮できれば、もう少し少ないLUT数でも動くかもしれません。

ちなみに、FPUを組み込むと、すごい勢いでLUTが消費されるので、覚悟しておいてください。

次にSDRAMが必須です。スタックとヒープ合わせて5MBが必要になるので、SDRAMなどの大容量メモリが必要になります。

最後に、これは必須ではないのですが、ブロックRAMは 250KB以上あると、レイトレをブロックRAM上に直接配置できて楽です。足りない場合は、SDカードなどからレイトレのプログラムを読み込むローダーが必要になります
-->
...
周辺機器としてUARTを備えていて、レイトレの結果をパソコンへ送ったりするのに利用します。
-->
...
プログラムは0番地からスタートするので、起動時に動かしたいプログラムは0番地のあるブロックRAM上へ配置する必要があります。

また、メモリマップトIOでUARTやLEDにアクセスすることができます。
-->
...
CPUの作り方に関しては、関連書籍がたくさんあるので、そちらをご参照ください。

私が参考にした書籍は、主に以下の三冊です。
...
<!--
FPUコアと、UARTコントローラと、SDRAMコントローラは github から持ってきたものを利用しました。

FPUコアとUARTコントローラはお好きなものをご利用ください、という感じなのですが、

SDRAMコントローラーに関しては、ご自分のFPGAに載っているSDRAMに適したものを探す必要があるので、気をつけてください。Xilinxのボードなら、MIG などを利用するのが良いんですかね？
-->
...
MinCamlコンパイラのビルド時に、CPUアーキテクチャを指定することで、出力するアセンブリコードを切り替えることができます。

今回は、この対応CPU一覧に「RISCーV」を追加しました
-->
---
...
<!--
MinCaml中のCPUアーキテクチャに依存したコードは、各CPUアーキテクチャごとのディレクトリの下に配置されます。

例えば、「PowerPC/emit.ml」には、アセンブリコード生成ロジックがまとめられていて、PowerPC のアセンブリコードを出力するためのコードがここにまとまっています。
-->
...
<!--
MinCamlをRISC-Vへ移植する流れは、

1. まず PowerPC ディレクトリを複製して、RV32 ディレクトリを作成します

これだけだと、PowerPC のアセンブリコードが出力されてしまうので、

2. で asm.ml で定義されているレジスタ一覧を、RISC-Vのものに置き換えたり、

3. で PowerPC のアセンブリコードを RISC-V のアセンブリに置き換えたり、

4. で、PowerPC アセンブリで書かれたライブラリ関数を、RISC-V アセンブリで描き直したりします。

こんな流れで MinCaml の移植を行います。
-->
...
出力されたRISC-Vのアセンブリを、GNUツールチェインの「アセンブラ」や「リンカ」を使ってビルドし、
...
<!--
MinCamlはOCamlで書かれているため、OCamlのソースが読めないとMinCamlを移植することができません。

私はMinCamlを移植するためにOCamlをさわり始めたのですが、文法やモジュールシステムが独特で、なかなかとっつきにくい印象を受けました。

また、関数型言語の流儀や、再帰を多用したプログラミングスタイルに慣れていないのもあって、OCamlの習得には結構苦労しました。
...
<!--
また、OCamlはちょっと...という人は、Rust で書かれたMinCamlコンパイラがあり、こちらも昨年度のCPU実験から使用可能になったらしいので、こちらを試してみるのも良いかもしれません。
-->
...
アセンブリを書いていると、「C言語でどう書けばいいかは分かるけど、アセンブリでどう書けばいいか分からない！」ということがよくあります。
...
<!--
また、GCCの「-S」オプションを使って、Cのソースから、アセンブリコードが出力することができます。
...
<!--
オリジナルのMinCamlのライブラリ関数は、libmincaml.S というファイルにアセンブリで記述されています。

RISC-Vへのライブラリ関数の移植も、最初はアセンブリで実装していたのですが、途中で嫌になってMinCamlで書きました
-->
